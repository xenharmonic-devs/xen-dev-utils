...
function toQuoteCycle(value: any) {
...
describe('Fraction', () => {
  for (let i = 0; i < tests.length; i++) {
    ...
    let set: FractionValue;
    let set2: number | undefined;
    if (Array.isArray(testCase.set)) {
      set = testCase.set[0];
      set2 = testCase.set[1];
    } else {
      set = testCase.set as FractionValue;
      if ('set2' in testCase) {
        set2 = testCase.set2 as number;
      }
    }
    ...
    if (fn !== undefined || testCase.param !== undefined) {
      action = () => {
        let param: any = testCase.param;
        if (Array.isArray(param)) {
          param = new Fraction(param[0], param[1]);
        }
        param = toQuoteCycle(param);
        const x = (new Fraction(set, set2) as any)[fn ?? 'add'](param);
        return x;
      };
    } else {
      action = () => {
        const x = new Fraction(set, set2);
        return x;
      };
    }

    it(String(testCase.label || testCase.set), () => {
      if (testCase.expectError) {
        expect(action).toThrowError(testCase.expectError);
      } else {
        const x = action();
        if (x === null) {
          expect(testCase.expect).toBe('null');
        } else if (typeof x === 'number') {
          expect(Math.sign(x)).toBeCloseTo(
            testCase.expect as unknown as number,
          );
        } else if (x instanceof Fraction) {
          const expected = new Fraction(toQuoteCycle(testCase.expect));
          expect(
            x.equals(expected),
            `${x.toFraction()} != ${expected.toFraction()} = ${testCase.expect}`,
          ).toBe(true);
        } else {
          expect(x.toString()).toBe(testCase.expect);
        }
      }
    });
  }
...
describe('Arguments', () => {
  it.each([
    ...
    const fraction = new Fraction(param);
    expect(`${fraction.n}/${fraction.d}`).toBe(expected);
    expect(fraction.s).toBe(1);
  });
});

describe('fractions', () => {
  it('Should pass 0.08 = 2/25', () => {
    const fraction = new Fraction('0.08');
    expect('2/25').toBe(`${fraction.n}/${fraction.d}`);
  });
  it('Should pass 0.200 = 1/5', () => {
    const fraction = new Fraction('0.200');
    expect('1/5').toBe(`${fraction.n}/${fraction.d}`);
  });

  it('Should pass 0.125 = 1/8', () => {
    const fraction = new Fraction('0.125');
    expect('1/8').toBe(`${fraction.n}/${fraction.d}`);
  });

  it('Should pass 8.36 = 209/25', () => {
    const fraction = new Fraction(8.36).simplify();
    expect('209/25').toBe(`${fraction.n}/${fraction.d}`);
  });

  it('Should add complex values', () => {
    const fraction = new Fraction(-1023461776, 334639305);
    const sum = fraction.add({n: 4, d: 25});
    expect('-4849597436/1673196525').toBe(`${sum.s * sum.n}/${sum.d}`);
  });
});

describe('Fraction Output', () => {
  it('Should pass -1.0000000000 = -1', () => {
    const fraction = new Fraction('-1.0000000000');
    expect('-1').toBe(fraction.toFraction());
  });

  it('Should pass -0.0000000000 = 0', () => {
    const fraction = new Fraction('-0.0000000000');
    expect('0').toBe(fraction.toFraction());
  });

  it('Should pass 1/-99/293 = -1/29007', () => {
    const fraction = new Fraction(-99).inverse().div(293);
    expect('-1/29007').toBe(fraction.toFraction());
  });

  it('Should work with large calculations', () => {
    const x = new Fraction(1123875);
    const y = new Fraction(1238750184);
    const z = new Fraction(1657134);
    const r = new Fraction(77344464613500, 92063);
    expect(x.mul(y).div(z).toFraction()).toBe(r.toFraction());
  });
});

describe('Fraction toContinued', () => {
  it('Should pass 415/93', () => {
    const fraction = new Fraction(415, 93);
    expect('4,2,6,7').toBe(fraction.toContinued().toString());
  });

  it('Should pass 0/2', () => {
    const fraction = new Fraction(0, 2);
    expect('0').toBe(fraction.toContinued().toString());
  });

  it('Should pass 1/7', () => {
    const fraction = new Fraction(1, 7);
    expect('0,7').toBe(fraction.toContinued().toString());
  });

  it('Should pass 23/88', () => {
    const fraction = new Fraction('23/88');
    expect('0,3,1,4,1,3').toBe(fraction.toContinued().toString());
  });

  it('Should pass 1/99', () => {
    const fraction = new Fraction('1/99');
    expect('0,99').toBe(fraction.toContinued().toString());
  });

  it('Should pass 1768/99', () => {
    const fraction = new Fraction('1768/99');
    expect('17,1,6,14').toBe(fraction.toContinued().toString());
  });

  it('Should pass 1768/99', () => {
    const fraction = new Fraction('7/8');
    expect('0,1,7').toBe(fraction.toContinued().toString());
  });
});
...
describe('Fraction simplify', () => {
  it('Should pass 415/93', () => {
    const fraction = new Fraction(415, 93);
    expect('9/2').toBe(fraction.simplify(0.1).toFraction());
    expect('58/13').toBe(fraction.simplify(0.01).toFraction());
    expect('415/93').toBe(fraction.simplify(0.0001).toFraction());
  });
});
